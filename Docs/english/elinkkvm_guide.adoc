...
When first open `eLinkViwer`, the following dialog is presented:
...
Before a remote control session, eLinkViewer allows a user different
methods to interact with eLinkKVM. The interaction methods include
configurations for `Keyboard`, `Video` and `Mouse` are sent and
received between a remote terminal and a remote host. A disk image can
also be selected as a virtual media for a remote host to facilitate
data transfer.

==== ELink Configuration UI

To configure a remote controll session, open `ELink Configuration` Dialog:
