...
See <<elinkviewer-chap>> chapter shows how to run and using eLinkViewer.
[[elinkviewer-chap]]
=== eLinkViewer quick start

. Establishing a new connection to  `eLinkKVM` with `eLinkViewer`.
+
...
+
.Configurate eLinkKVM
image::../../Resource/EV_click_config.png[]
. Enable `Keyboard HID USB` and active the interface
+
.Keyboard Operation Modes
image::../../Resource/EV_config_kv_hidusb_active.png[]
. Enable `Mouse ABS USB` and active the interface
+
.Mouse Operation Modes
image::../../Resource/EV_config_mouse_absusb_active.png[]
. Set `Video Mode` to VGA
+
.Video Capture Modes
image::../../Resource/EV_config_VGA_active.png[]
+
